...
const getPropertyByKeys = async (url, ...keys) => {
  if (url === undefined) {
    return null;
  }
...
const getImageUrlByType = async (type) => {
  const urlsByType = [
    {
  ...
  const urlType = urlsByType.filter((urlObj) => urlObj.type === type)[0];

  if (!urlType) {
    throw Error("Unavaiable type");
  }

  return await getPropertyByKeys(urlType.url, urlType.keys);
};

const loadImageByType = async (type) => {
  try {
    const url = await getImageUrlByType(type);
...
const setImages = async (element) => {
  element.appendChild(await loadImageByType("cat"));
  element.appendChild(await loadImageByType("dog"));
  element.appendChild(await loadImageByType("lion"));
  element.appendChild(await loadImageByType());
};
