const getDefaultErrorImage = () => {
  ...
  img.setAttribute("alt", "default error image");
  return img;
};

const loadImageWithDefaultError = (url) => {
  if (url === undefined) {
    return Promise.resolve(getDefaultErrorImage());
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    req.addEventListener("load", loadImgCallback);
    req.addEventListener("error", (error) => reject(Error("Network Error")));
    req.send();
  });
};

const rejectImageImmediatly = (url) => {
  if (url === undefined) {
...
// loadImageWithDefaultError().then((resultImage) =>
//   imagesList.appendChild(resultImage)
// );

// loadImageWithDefaultError(url).then((resultImage) =>
//   imagesList.appendChild(resultImage)
// );

rejectImageImmediatly().then(
  (resultImage) => imagesList.appendChild(resultImage),
  (reason) => imagesList.append(reason)
);

rejectImageImmediatly(url).then(
  (resultImage) => imagesList.appendChild(resultImage),
  (reason) => imagesList.append(reason)
);
